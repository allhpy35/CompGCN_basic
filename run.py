	...
	def predict(self, split='valid', mode='tail_batch'):
		...
		with torch.no_grad():
			...
			for step, batch in enumerate(train_iter):
				...
				## 정답 entity(obj)가 전체 후보 entity 중에서 “몇 등(rank)”인지 계산
				## 성적순으로 줄 정렬(argsort 안쪽) -> 각 학생들 등수 기입하기(argsort 밖에)
				## +1은 index로 된걸 rank로 바꾸기 위해서
				ranks			= 1 + torch.argsort(torch.argsort(pred, dim=1, descending=True), dim=1, descending=False)[b_range, obj]

				ranks 			= ranks.float()

				## torch.numel(x) : number of elements in input tensor "x"
				results['count']	= torch.numel(ranks) 		+ results.get('count', 0.0)



				## torch.sum() Vs torch.sum().item() ?
				## torch.sum() : Output type is Tensor -> cuda, gpu available!
				## torch.sum().item() : Output type is Scalar -> float, int type

				## results.get? : dict.get(key, default)
				## 초기에는 key 'mr'에 값이 없기 때문에 default value 인 0.0을 가져옴
				## ""지금 results.get 해서 mr key의 value를 게속 더하게 되어있음""
				## 1 epoch 돌기까지 게속 더해지는 것.
	...
	def fit(self):
		...
		for epoch in range(self.p.max_epochs):
			...
			if val_results['mrr'] > self.best_val_mrr:
				...
				'''
					gamma는 TransE 계열의 margin-based loss를 위한 파라미터이고,
					지금 네가 쓰는 ConvE 설정에서는 사실상 의미가 없다.
				'''
